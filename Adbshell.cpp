...
BOOL CAdbshell::Start(CString strFileName, CString IpAdress, int type)
{
	LPSTR lpPath;
	switch (type)
	{
	case 0:
		strFileName += " devices";
		lpPath = strFileName.GetBuffer();
		strFileName.ReleaseBuffer();
		break;
	case 1:
		strFileName += " -s ";
		strFileName += IpAdress;
		strFileName += " shell";
	...
	if (CreateProcess(NULL, lpPath, NULL, NULL, TRUE, 0, NULL, NULL, &startupinfo, &pinfo) == FALSE) {
