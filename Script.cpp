...
void CScript::DealyRandTime(unsigned long minDeaySeconds = 0, unsigned long maxDealySeconds = 10)
{
	...
	WriteLog(str, true, BLUECOLOR,false);
	resultRand *= 1000;
	Dealy(resultRand);
}
...
int CScript::SendXSolider(int type)
{
	...
	VARIANT x, y;
	CString str;
	str.Format("solider_%d.bmp", type);
	//WriteLog(str,true,REDCOLOR);
...
int CScript::checkMainScreen()
{
	...
	do
	{
		...
		if (waitTime % 5 == 0)
		{
			WriteLog(showStr,true,BLUECOLOR,false);
		}
		else
		{
			WriteLog(showStr,false,BLUECOLOR,false);
		}
		if (waitTime >= 60)
		{
			scriptStateCode = ShouldRestart;
			WriteLog(_T("未发现村庄，尝试重启"),true,REDCOLOR,true);
			return 0;
		}
		if (scriptStateCode <= 0)
		{
...
int CScript::MakeArmy()
{
	...
	CString PicName;
	PicName = dm.MatchPicName("main_screen*.bmp");
	if (FindPicSuper(256, 1, 324, 62, "\Pic\\others", PicName, 10, false)!=1)
	{
		return 0;
	}
	...
	dm.LeftClick();
	long ex = 0, ey = 0;
	if (FindPicSuper(785, 47, 850, 138, "\Pic\\others", "Esc_Army.bmp", 50, false) != 1)
	{
		WriteLog("FindPicSuper false", true, REDCOLOR, false);
		return 0;
	}
	dm.MoveTo(295, 96);
	Dealy(20);
	...
	if (TrainArmyStyle == 0)
	{
		...
		for (int i = 1; i <= 12; i++)
		{
			...
			{
				WriteLog("Not Find  " + str_i,true,REDCOLOR,false);
			}
		}
	...
	if (_ttoi(coc.GetSingleSetting("IsSpeedUp"))==1)
	{
		SpeedTrain();
	}
	if (TrainArmyStyle >= 1)
	{
		...
		if (x.lVal <= 0)
		{
			WriteLog("error:bad one key train!",false,REDCOLOR,false);
		}
	}
	...
	dm.SetPath("\Pic\\others");
	if (FindPicSuper(775, 60, 844, 149,"\Pic\\others","Esc_Army.bmp",30,true)!=1)
	{
		dm.MoveTo(812, 94);
		Dealy(20);
		dm.LeftClick();
...
int CScript::adbCmd(CString f_input)
{
	...
	if (adb_error.Find("error: device not found") >= 0)
	{
		...
		{
			WriteLog(_T("error:kill adb.exe 失败 请检查Adb 路径！ "), true, RGB(0xff, 0x00, 0x00), false);
		}
		adb.Start(path, strIp, 1);
		adb.RunCmd(f_input);
		adb.Stop();
...
int CScript::Main_Attack()
{
	...
	AttackChangeDealy= _ttoi(coc.GetSingleSetting("AttackGird"));
...
int CScript::EqualAttack()
{
	...
	AttackChangeDealy = (AttackChangeDealy + 1) * 10;
	dm.SetPath(_T("\Pic\\attack"));
	WriteLog(_T("定位下兵位置"),true,BLUECOLOR,false);
	//VARIANT x1, y1;
	int solider_num = 0; //
	int distance = 1;
	int attackcount = 0;
	...
	WriteLog(_T("定位完成"),true,BLUECOLOR,false);
	Dealy(200);
	GetArmyMsg();
	VARIANT xg, yg;
	for (int j = 0; j <= 3; j++)
	{
		...
		{
			Dealy(50);
			for (int i = 0; i < solider_num/(4-j); i += 1)
			{
				dm.MoveTo(x[j][i], y[j][i]);
				Dealy(AttackSpeed);
				dm.LeftClick();
		...
		if (xg.lVal > 0)
		{
			WriteLog("释放援军",true,BLUECOLOR,false); dm.MoveTo(xg.lVal, yg.lVal); Dealy(20); dm.LeftClick(); Dealy(200); dm.MoveTo(x[0][1], y[0][1]); Dealy(20); dm.LeftClick();
		}
		dm.FindMultiColor(18, 568, 834, 617, "e7b463-0f0f0f", "21|13|e8a540-0f0f0f,-7|13|e0bf76-0f0f0f,4|9|e89268-0f0f0f,8|10|ea946e-0f0f0f", 0.9, 1, &xg, &yg);//bar barin king
		if (xg.lVal > 0)
		{
			WriteLog("释放蛮王",true, BLUECOLOR, false); dm.MoveTo(xg.lVal, yg.lVal); Dealy(20); dm.LeftClick(); Dealy(200); dm.MoveTo(x[0][1], y[0][1]); Dealy(20); dm.LeftClick();
		}

		dm.FindMultiColor(18, 568, 834, 617, "6C4CB4-040404", "0|0|6C4CB4-040404,4|-1|7253BC-060704,-8|16|D59486-0B1012,7|15|E48A61-040202,8|12|F39A6E-030503,11|12|F4A074-040404,23|13|6C44B6-040202,26|21|6840B0,31|21|6C48BB-040303", 0.9, 1, &xg, &yg);//archer king
		if (xg.lVal > 0)
		{
			WriteLog("释放女王",true, BLUECOLOR, false);
			dm.MoveTo(xg.lVal, yg.lVal);
			Dealy(20);
			dm.LeftClick();
			Dealy(200);
	...
	WriteLog("释放剩余兵种", true, BLUECOLOR, false);
	for (int j = 0; j <= 3; j++)
	{
		if (scriptStateCode <= 0)
		{
		...
		if (solider_num > 0)
		{
			WriteLog(_T("giant"), true, BLUECOLOR, false);
			Dealy(100);
			for (int i = 0; i < solider_num / (4 - j); i++)
			{
				dm.MoveTo(x[j][i], y[j][i]);
...
int CScript::WriteLog(CString logStr, bool IsShow, COLORREF color, bool IsSave)
{
	...
	if (IsShow==true)
	{
		...
		int x = pRichEditCtrl->GetLineCount() - 1;
		...
		{
			WriteStr = "out of maxLength!";
		}
		if (x >= 1000)
		{
			pRichEditCtrl->SetWindowText("");
		}
		pRichEditCtrl->SetSel(-1, -1);
		pRichEditCtrl->ReplaceSel(WriteStr, 0);
		int lineStart = pRichEditCtrl->LineIndex(x);//取第x行的第一个字符的索引
		int lineEnd = pRichEditCtrl->LineIndex(x + 1) - 1;//取第x行的最后一个字符的索引——用第x+1行的第一个索引减1来实现
		pRichEditCtrl->SetSel(lineStart, lineEnd);//选取第一行字符
		pRichEditCtrl->SetSelectionCharFormat(cf);//设置颜色
		pRichEditCtrl->PostMessage(WM_VSCROLL, SB_BOTTOM, 0);
	}
	if (IsSave==true)
	{
		CString path;
...
{
	WriteLog(_T("搜鱼"), true, BLUECOLOR, false);
	dm.MoveTo(59, 601);
	Dealy(200);
	dm.LeftClick();
	Dealy(2000);
	...
	do
	{
		...
		do
		{
			...
			{
				WriteLog(_T("卡白云时间过长！尝试重新启动"), true, REDCOLOR, false);
				scriptStateCode = -2;
				IsThreadRun = false;
				return 0;
			}
		...
		for (int i=0;i<maxStrLen-SearchCount_str.GetLength();i++)
		{
			SearchCount_str += " ";
		}
		SearchCount_str += "| ";
		showStr = SearchCount_str;
		showStr += "[金] ";
		showStr += G_gold;
		for (int i=0;i<maxStrLen-G_gold.GetLength();i++)
		{
			showStr += " ";
		}
		showStr += "[水] " + G_water;
		for (int i = 0; i < maxStrLen - G_oil.GetLength(); i++)
		{
			showStr += " ";
		}
		showStr += "[油] " + G_oil;
		...
		if (SearchResult(T_gold, T_water, _ttoi(G_gold), _ttoi(G_water), type) > 0 && _ttoi(G_oil) >= T_oil)
		{
			if (check_deadbase() >= 1)
			{
				...
				if (check_defends() >= 1)
				{
					...
					Dealy(200);
					dm.LeftClick();
					WriteLog(_T("防御过高"), true, BLUECOLOR, false);
					Dealy(1000);
				}
			}
			else
			{
				...
				Dealy(200);
				dm.LeftClick();
				WriteLog(_T("不是死鱼"), true, BLUECOLOR, false);
				Dealy(1000);
			}
		}
		else
...
{
	WriteLog("收集资源", true, BLUECOLOR, false);
	VARIANT x, y;
	for (int s = 1; s <= 10; s++)
	{
...
int CScript::Donate()
{
	WriteLog("捐兵", true, BLUECOLOR, false);
	...
	for (long donate_y = 89; donate_y < 666; donate_y += 50)
	{
		...
		if (xs.lVal > 0)
		{
			...
			if (s != "")
			{
				WriteLog(_T("需要:") + s, true, BLUECOLOR, false);
			}
			else
			{
				continue;
		...
		else
		{
			...
			if (xs.lVal > 0 && ExtraClick < 10&&donate_y>=600)
			{
				donate_y = 89;
...
int CScript::UpGrade_Wall()
{
	...
	WriteLog(wall_pic , true, BLUECOLOR, false);
...
int CScript::SetClientWindowSize()
{
	...
	if ((xw.lVal != 850) || (yw.lVal != 667))
	{
		...
		WriteLog("错误的分辨率：" + str, true, BLUECOLOR, false);
...
{
	adbRunApp(_in_coc_ver);
	WriteLog("启动部落冲突", true, BLUECOLOR, false);
	return 0;
}
...
int CScript::StopCoc(CString _in_coc_ver)
{
	...
	adbCloseApp(close_ver);
	WriteLog("停止部落冲突", true, BLUECOLOR, false);
	return 0;
}
...
{
	WriteLog(_T("降杯"), true, REDCOLOR, false);
	dm.MoveTo(59, 601);
	Dealy(200);
	dm.LeftClick();
	Dealy(2000);
...
int CScript::ZoomOut()
{
	...
	adbCmd("input swipe 400 100 400 600");
	return 1;
}
...
int CScript::GetArmyMsg()
{
	...
	if (type == 0)//进行识别
	{
		...
		const int minY = 572;
		const int maxY = 597;
		WriteLog("识别军队信息：",true,BLUECOLOR,false);
		dm.UseDict(6);
		VARIANT x, y;
		CString str;
...
long CScript::FindPicSuper(long x1,long y1,long x2,long y2,LPCTSTR path,LPCTSTR picName,int timesOut,bool Isclick)
{
	VARIANT x, y;
	int index = 0;
	long result = 0;
	...
	do 
	{
		...
		if (index>MAXFINDPICTIME||timesOut<0)
		{
			break;
		}
		else
...
{
	CString str;
	str.Format("input swipe %d %d", x1, y1, x2, y2);
	adbCmd(str);
	return 0;
}
...
{
	CString str;
	str.Format("input keyevent %d %d", keyCode);
	adbCmd(str);
	return 0;
}
...
{
	adbCmd("am start -n " + bagNameAndClassName);
	return 0;
}
...
{
	adbCmd("am force-stop" + packageName);
	return 0;
}
